...
export class ServiceRequest extends fhir.DomainResource {
  ...
  constructor(source:Partial<ServiceRequestArgs> = {}, options:fhir.FhirConstructorOptions = {}) {
    ...
    if (source['requisition']) { this.requisition = new fhir.Identifier(source.requisition, options); }
    ...
    if (source['orderDetail']) { this.orderDetail = source.orderDetail.map((x) => new fhir.CodeableConcept(x, options)); }
    ...
    else if (source['quantityRatio']) { this.quantity = new fhir.Ratio(source.quantityRatio, options); }
    ...
    if (source['locationCode']) { this.locationCode = source.locationCode.map((x) => new fhir.CodeableConcept(x, options)); }
    else { this.locationCode = []; }
    if (source['locationReference']) { this.locationReference = source.locationReference.map((x) => new fhir.Reference(x, options)); }
