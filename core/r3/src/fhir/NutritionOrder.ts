...
export class NutritionOrder extends fhir.DomainResource {
  ...
  constructor(source:Partial<NutritionOrderArgs> = {}, options:fhir.FhirConstructorOptions = {}) {
    ...
    if (source['status']) { this.status = new fhir.FhirCode<NutritionRequestStatusCodeType>({value: source.status}); }
    if (source['_status']) {
      if (this.status) { this.status.addExtendedProperties(source._status!); }
      else { this.status = new fhir.FhirCode<NutritionRequestStatusCodeType>(source._status as Partial<fhir.FhirCode>); }
    }
