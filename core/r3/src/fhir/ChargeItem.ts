...
export class ChargeItem extends fhir.DomainResource {
  ...
  constructor(source:Partial<ChargeItemArgs> = {}, options:fhir.FhirConstructorOptions = {}) {
    ...
    if (source['definition']) { this.definition = source.definition.map((x) => new fhir.FhirUri({value: x})); }
    ...
    if (source['participant']) { this.participant = source.participant.map((x) => new fhir.ChargeItemParticipant(x)); }
