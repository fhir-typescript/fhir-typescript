/*! @fhir-typescript/r4-core v0.0.11-beta.2 2022-05-18 */
'use strict';
import { __extends } from '../_virtual/_tslib.js';
import { FhirBase } from './FhirBase.js';
import { FhirString } from './FhirString.js';
import { Extension } from './Extension.js';

// <auto-generated/>
/**
 * Base definition for all elements in a resource.
 */
var FhirElement = /** @class */ (function (_super) {
    __extends(FhirElement, _super);
    /**
     * Default constructor for FhirElement - initializes any required elements to null if a value is not provided.
     */
    function FhirElement(source, options) {
        if (source === void 0) { source = {}; }
        if (options === void 0) { options = {}; }
        var _this = _super.call(this, source, options) || this;
        if (source['id']) {
            _this.id = new FhirString({ value: source.id });
        }
        if (source['extension']) {
            _this.extension = source.extension.map(function (x) { return new Extension(x); });
        }
        else {
            _this.extension = [];
        }
        return _this;
    }
    /**
     * Function to perform basic model validation (e.g., check if required elements are present).
     */
    FhirElement.prototype.doModelValidation = function () {
        var _a;
        var outcome = _super.prototype.doModelValidation.call(this);
        if (this["id"]) {
            (_a = outcome.issue).push.apply(_a, this.id.doModelValidation().issue);
        }
        if (this["extension"]) {
            this.extension.forEach(function (x) {
                var _a;
                (_a = outcome.issue).push.apply(_a, x.doModelValidation().issue);
            });
        }
        return outcome;
    };
    /**
     * Mapping of this datatype to a FHIR equivalent
     */
    FhirElement._fts_dataType = 'Element';
    return FhirElement;
}(FhirBase));

export { FhirElement };
//# sourceMappingURL=FhirElement.js.map
